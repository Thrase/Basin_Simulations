...
#=
function operators_dynamic(p, Nr, Ns, μ, ρ, R, B_p, faces, metrics, 
                     τscale = 2,
                     crr = metrics.crr,
                     css = metrics.css,
                     crs = metrics.crs)
    ...
    @printf "\tGot Ã_rr in %f s\n" Ãrr_t
    flush(stdout)
    
    Ãss_t = @elapsed begin
    ...
    @printf "\tGot Ã_ss in %f s\n" Ãss_t
    flush(stdout)

    A_cross = @elapsed begin
        Ãsr = sparse(QsT ⊗ Ir) * sparse(1:length(crs), 1:length(crs), view(crs, :)) * sparse(Is ⊗ Qr)
        Ãrs = sparse(Is ⊗ QrT) * sparse(1:length(csr), 1:length(csr), view(csr, :)) * (Qs ⊗ Ir)
    end

    @printf "\tGot Ã_sr and Ãrs in %f s\n" A_cross
    flush(stdout)
    ...
    @printf "\tGot boundary related operators in %f s\n" boundary_t
    flush(stdout)
